...
def test_init():
    ...
    # Test returned object instance attributes are set to the default values.
    Jinja = JinjaUtils()
...
def test_init_verbose_log(capsys):
    """ JinjaUtils Class Verbose Enable Test

    This test will test the verbose method to ensure that when verbose
    is set to true, logs are written to stdout, stderr
    """
...
def test_init_verbose_false(capsys):
    """ JinjaUtils Class Verbose Disable Test

    This test will test the verbose method to ensure that when verbose is set
    to false no logs other then errors are output.
    """
...
def test_init_verbose_invalid(capsys):
    """ JinjaUtils Class Verbose Invalid Value Test

    This test will test if the verbose option is set to anything other
    then a bool, that the default value of false gets set and the bad
    type is ignored.
    """
...
def test_init_log_object(capsys):
    ...
    if the logs constructor object is provided a log object
    to write to that all class logs will be written to that provided
    ...
    # Instantiate a JinjaUtils object,
    # Test the returned object instance for expected values.
...
def test_init_log_invalid_object(capsys):
    ...
    # Write a test log entry
    JinjaBadLog.log("Pytest debug log write test", 'debug', 'test_log')
    JinjaBadLog.log("Pytest info log write test", 'info', 'test_log')
    JinjaBadLog.log("Pytest warning log write test", 'warning', 'test_log')
    JinjaBadLog.log("Pytest error log write test", 'error', 'test_log')
...
def test_verbose_property(capsys):
    """ JinjaUtils Class Verbose Property Test
...
def test_template_directory_property(capsys):
    """ JinjaUtils Class Template Directory Property Test

    This test will test the getters and setter methods
    for the template_directory property.
    """
...
def test_template_directory_property_invalid(capsys):
    ...
    This test will test the getters and setter methods or the
    template_directory property if an invalid value is passed to the
    property.
    """
    ...
    ###############################################
    # To clear the environment for max fail conditions,
...
def test_load(capsys):
    ...
    # jinja template library, and try to load it.
...
def test_render(capsys):
    """ JinjaUtils Class Jinja Render Method Test

    This test will test the getters and setter methods
    for the object render property.
    """
...
def test_render_exception(capsys):
    """ JinjaUtils Class Jinja Render Method Exception Tests

    This test will test the getters and setter methods
    for the object render property and test passing it bad values
    that will cause the render method to hit expected exception conditions.
    """
    ...
    ####################################################
    # Reference undefined template variable exception: #
    ####################################################
    # Load the first template and test the things.
    Jinja.load = 'test_tpl.j2'
    assert(Jinja._loaded_template.filename.endswith('test_tpl.j2'))
...
def test_write(capsys):
    """ JinjaUtils Class Jinja Write Method Test
    ...
    write_template = Jinja.write(
        output_directory=test_template_directory,
        output_file='test_tpl.j2', backup=False
    )
    # Find the backup file
    backup_file = None
    for root, dirs, files in os.walk(test_template_directory):
    ...
    write_template = Jinja.write(
        output_directory=test_template_directory,
        output_file='test_tpl.j2', backup=42
    )
